...
{
    public class GetJobSeekerSkillsForJobIdController : ApiController
    {

        public IEnumerable<SkillSmart.Dto.ListingJobSeekerSkills> GetAllJobSeekersSkillsListForSkillId(String jobId)
        {
            var alljobSkills = ServiceFactory.GetJobSkills().GetAll(jobId);
            List<string> jobSkillsList = new List<string>();
            foreach (JobSkills jobskill in alljobSkills)
            {
                jobSkillsList.Add(jobskill.SkillMapId);
            }
            string[] jobskillsid = jobSkillsList.ToArray();
            string id = String.Join(",", jobskillsid);
